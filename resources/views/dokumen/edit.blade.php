        ...
        <div class="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
            ...
            <div class="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 truncate max-w-md">{{ $dokumen->title }}</p>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 gap-x-5 gap-y-4 px-6 py-6 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="number" name="number" type="text"
                               value="{{ old('number', $dokumen->number) }}"
                        ...
                        <div class="relative">
                            <select id="category" name="category"
                                ...
                                @foreach($categories as $cat)
                                    <option value="{{ $cat }}" {{ old('category', $dokumen->category) === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="owner" name="owner"
                                    class="w-full appearance-none rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 outline-none transition-colors focus:border-red-700 focus:ring-2 focus:ring-red-100">
                                <option value="Internal BPA"  {{ old('owner', $dokumen->owner) === 'Internal BPA'  ? 'selected' : '' }}>Internal BPA</option>
                                <option value="Eksternal BPA" {{ old('owner', $dokumen->owner) === 'Eksternal BPA' ? 'selected' : '' }}>Eksternal BPA</option>
                            </select>
                            <i data-lucide="chevron-down" class="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400"></i>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input id="effective_date" name="effective_date" type="date"
                               value="{{ old('effective_date', $dokumen->effective_date?->format('Y-m-d')) }}"
                               class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-800 outline-none transition-colors focus:border-red-700 focus:ring-2 focus:ring-red-100">
                    </div>
                    ...
                    <div>
                        ...
                        <input id="version" name="version" type="text"
                               value="{{ old('version', $dokumen->version) }}"
                               class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-800 outline-none transition-colors focus:border-red-700 focus:ring-2 focus:ring-red-100">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="status" name="status"
                                ...
                                <option value="Tidak Aktif" {{ old('status', $dokumen->status) === 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            <i data-lucide="chevron-down" class="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400"></i>
                        </div>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-800 outline-none transition-colors placeholder:text-gray-400 focus:border-red-700 focus:ring-2 focus:ring-red-100 resize-none">{{ old('description', $dokumen->description) }}</textarea>
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="klausul" name="klausul" rows="3"
                                  class="w-full rounded-lg border border-gray-200 px-3 py-2.5 text-sm text-gray-800 outline-none transition-colors focus:border-red-700 focus:ring-2 focus:ring-red-100 resize-none">{{ old('klausul', $dokumen->klausul) }}</textarea>
                    </div>
                        ...
                        @if($dokumen->file_path)
                            <div class="mb-3 flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-blue-800 truncate">{{ basename($dokumen->file_path) }}</p>
