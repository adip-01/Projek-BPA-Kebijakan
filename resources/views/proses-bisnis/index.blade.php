                ...
                <article class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="flex h-48 items-center justify-center bg-gray-100 overflow-hidden">
                        @if($process->image_path)
                            <img src="{{ Storage::url($process->image_path) }}"
                                 alt="{{ $process->title }}"
                    ...
                    <div class="p-4">
                        <h3 class="font-bold text-gray-800">{{ $process->title }}</h3>
                        @if($process->description)
                            <p class="mt-1 text-sm text-gray-500 line-clamp-2">{{ $process->description }}</p>
                        @endif

                        <div class="mt-4 flex items-center justify-between">
                            <span class="text-xs text-gray-400">Diperbarui {{ $process->updated_at->diffForHumans() }}</span>
                            <div class="flex items-center gap-1">
                                <button type="button"
                                        onclick="openEditModal({{ $process->id }}, '{{ addslashes($process->title) }}', '{{ addslashes($process->description ?? '') }}')"
                                ...
                                <form action="{{ route('proses-bisnis.destroy', $process) }}" method="POST"
...
<div id="modal-add-process" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative z-10 w-full max-w-lg rounded-xl bg-white shadow-xl">
        ...
        <form action="{{ route('proses-bisnis.store') }}" method="POST" enctype="multipart/form-data" class="px-6 py-5 space-y-4">
            @csrf
            <div>
