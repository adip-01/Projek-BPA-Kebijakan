...
@section('breadcrumb', 'Dokumen SPMI')

@section('content')
<div x-data="dokumenSpmiAdmin()">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm
            ...
            <div>
                <p class="text-[11px] font-bold text-gray-400 uppercase
                          tracking-wider">STANDAR</p>
                <p class="text-3xl font-extrabold text-gray-900 leading-tight">
                    {{ str_pad($countStandar ?? 0, 2, '0', STR_PAD_LEFT) }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm
            ...
            <div>
                <p class="text-[11px] font-bold text-gray-400 uppercase
                          tracking-wider">KEBIJAKAN</p>
                <p class="text-3xl font-extrabold text-gray-900 leading-tight">
                    {{ str_pad($countKebijakan ?? 0, 2, '0', STR_PAD_LEFT) }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm
            ...
            <div>
                <p class="text-[11px] font-bold text-gray-400 uppercase
                          tracking-wider">MANUAL</p>
                <p class="text-3xl font-extrabold text-gray-900 leading-tight">
                    {{ str_pad($countManual ?? 0, 2, '0', STR_PAD_LEFT) }}
                </p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm
            ...
            <div>
                <p class="text-[11px] font-bold text-gray-400 uppercase
                          tracking-wider">FORMULIR</p>
                <p class="text-3xl font-extrabold text-gray-900 leading-tight">
                    {{ str_pad($countFormulir ?? 0, 2, '0', STR_PAD_LEFT) }}
                </p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-50" id="spmiTableBody">
                    ...
                    @php
                        ...
                        $jenis = strtoupper($dok->jenis_dokumen ?? '');
                        $c = $jenisBadge[$jenis] ?? [
                            'bg'   => 'bg-gray-100',
                            'text' => 'text-gray-700',
                            'icon' => 'text-gray-400 bg-gray-50',
                        ];
                    @endphp

                    <tr class="hover:bg-gray-50 transition-colors spmi-row"
                        data-nama="{{ strtolower($dok->nama_dokumen ?? '') }}">

                        {{-- Nama / Judul --}}
                        <td class="px-5 py-3.5">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg {{ $c['icon'] }}
                    ...
                    <tr>
                        <td colspan="5"
                            ...
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <p class="font-medium">Belum ada dokumen SPMI.</p>
        ...
        <div class="px-5 py-4 border-t border-gray-100
            ...
            <p class="text-xs text-gray-500">
                Halaman
                <span class="font-semibold">{{ $dokumens->currentPage() }}</span>
                dari
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm
            ...
            <div class="px-8 py-8">
                <p class="text-lg font-semibold text-gray-800 mb-6">
                    Anda yakin ingin menhapus dokumen ?
                </p>
...
function dokumenSpmiAdmin() {
    return {
        ...
        openEdit(id, namaDokumen, nomorDokumen, jenisDokumen) {
            this.resetEditForm();
            this.editForm.id            = id;
        ...
        async submitTambah() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    this.showAlertMsg(
                        'success',
                        json.message || 'Dokumen SPMI berhasil ditambahkan.'
                    );
        ...
        async submitEdit() {
            ...
            try {
                const url =
                    `{{ url('dashboard/dokumen-spmi') }}/${this.editForm.id}`;
                ...
                if (res.ok) {
                    ...
                    this.showAlertMsg(
                        'success',
                        json.message || 'Dokumen SPMI berhasil diperbarui.'
                    );
                    setTimeout(() => location.reload(), 900);
                } else {
                    const { errors, fieldErrors } = await this.parseErrors(res);
        ...
        async submitHapus() {
            ...
            try {
                const url =
                    `{{ url('dashboard/dokumen-spmi') }}/${this.hapusForm.id}`;
                ...
                if (res.ok) {
                    ...
                    this.showAlertMsg(
                        'success',
                        json.message || 'Dokumen SPMI berhasil dihapus.'
                    );
                    setTimeout(() => location.reload(), 900);
                } else {
        ...
        init() {
            ...
            if (input) {
                input.addEventListener('input', function () {
                    ...
                    const rows = document.querySelectorAll('.spmi-row');
                    rows.forEach(row => {
